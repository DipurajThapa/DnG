...
contains(app,["reports:'Reports'",'home.notifications','project_capacity_projection','planned_cost_projection','snapshot_project_report','No separate status dataset','No actual cost, margin or EAC'],'core UI');
contains(gov,["new Set(['requirements','raid','change'])",'project_control_extensions','create_requirement','baseline_requirement','link_requirement','accept_requirement','create_raid_item','evaluate_raid_triggers','convert_raid_trigger_to_issue','submit_initial_baseline','finalize_initial_baseline','create_change_request','prepare_change_request','finalize_change_request','Human-confirmed links count','Risk scoring is human-entered','Approved baselines are immutable snapshots'],'governance UI');
contains(pmo,['integration_health','outcome_metrics','record_admin_effort_sample','Only evidence-supported measures are calculated','Do not estimate a lower number','M1_adminEffort','M4_decisionLatency','M8_evidenceCoverage','M9_dataFreshness','M12_notificationNoise'],'PMO controls');
contains(roles,['My responsibilities','admin_role_coverage','acceptance_readiness','Configuration is not treated as proof','Only responsibilities assigned to your verified identity appear here','Do not grant your own login every role merely to preview screens','goliathInviteRoleHolder'],'role model UI');
...
contains(invitationFlow,['admin_invitation_targets','admin_create_identity_invitation','v_reused','admin_record_identity_invitation_share','copy-token','email-client'],'invitation backend');
...
const migrationFiles=[
  ...
  '20260912_role_action_capability_matrix.sql',
  '20260912_role_action_capability_matrix_fix.sql',
  '20260912_role_action_data_action_fix.sql',
  '20260912_authorization_self_test.sql',
  '20260912_capability_enforcement_wrappers.sql',
  '20260912_remaining_action_policy.sql',
  '20260912_remaining_capability_wrappers.sql',
  '20260912_final_mutation_policy.sql',
